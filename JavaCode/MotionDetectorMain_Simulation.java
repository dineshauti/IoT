...
public class MotionDetectorMain_Simulation implements Runnable{
	...
	public void run() {
		...
		try{
            ...
            PreparedStatement prep = con.prepareStatement("SELECT door_status FROM motiondetector");
            ...
            if (currentStatus != door2.getDoorStatus())
            {
            	if (door2.getDoorStatus() == 1)
            		System.out.println("Motion Detector Activated");
            	else
            		if (door2.getDoorStatus() == 0)
            		System.out.println("Motion Detector Deactivated");
